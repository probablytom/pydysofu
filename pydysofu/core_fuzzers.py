...
# Decorator which takes an argument, function invocation_monitor_and_mutator
# The monitor is itself generated by invocation_recorder_producer below.
def result_recorder(invocation_monitor_and_mutator):

    # The decorator itself
    def record_result(func):

        # A function which records the output of a function via the monitor arg
        def _recorder(*args, **kwargs):
            output = func(*args, **kwargs)
            invocation_recorder_producer(get_reference_syntax_tree(func), output)
            return output

        return _recorder


def invocation_recorder_producer(update_steps_func, fitness_func, max_trials):
    '''
    A function which produces a generator function which records mutations of a
    given function, and permenantly mutates it after a given maximum number of
    mutations. Intended for genetic algorithms etc.
    '''
    def record_invocation_result(steps, output):
        history_of_outputs = []
        invocation_count = 0
        best_steps = steps
        while True:
            # Calling this generator replaces steps and output, but not best_steps

            history_of_outputs.append((steps, output))
            invocation_count += 1

            # Mutate permentantly if we've reached the required number of execs
            if invocation_count % max_trials == 0:
                metrics = [(fitness_func(result), corresponding_func)
                           for (corresponding_func, result) in history_of_outputs]
                best_pair = max(metrics.sort(key=lambda x: x[1]))
                best_steps = best_pair[0]
                update_steps_func(best_steps)  # Send the new best steps back to incrementally_improve()

                history_of_outputs = []

            # Hand control back, while keeping state
            yield

    return record_invocation_result


def incrementally_improve(mutator, fitness_func=lambda x: x, number_of_trials=10):

    def __incrementally_improve(steps, context):

        best_steps = steps

        def recieve_message(new_best_steps=steps):
            best_steps = new_best_steps

        invocation_recorder = invocation_recorder_producer(recieve_message, fitness_func, number_of_trials)
        result_recorder_decorator = result_recorder(invocation_recorder)

        while True:

            # TODO: Apply the above decorator to best_steps before we mutate

            yield mutator(best_steps, context)
