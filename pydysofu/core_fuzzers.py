...
def incrementally_improve(mutator, fitness_function=lambda x: x, max_trials=10):

    # Namespace hack so as not to hit scoping issues later
    class Namespace:
        pass
    _ = Namespace()
    _.best_steps = None
    ...
    def wrap_to_catch_value(func, mutated_steps):
        def _wrapper(*args, **kwargs):
            result = func(*args, **kwargs)

            _.invocation_history[mutated_steps] = result

            # Choose a new best set of steps if it's time
            if len(_.invocation_history.items()) % max_trials == 0:
                zipped_history = _.invocation_history.items()

                # Sort the history by the value of the fitness function
                # applied to the res part. Sorting goes lowest to highest.
                zipped_history.sort(key=lambda x: fitness_function(x[1]))
                _.best_steps = zipped_history[-1][0]
    ...
    def _incrementally_improve_fuzzer(steps, context):
        # If we've never run before, set up an initial set of best steps.
        if _.best_steps is None:
            _.best_steps = steps

        steps = mutator(_.best_steps)
        new_wrapped_function = wrap_to_catch_value(compile(steps), steps)
        wrapped_steps = get_reference_syntax_tree(new_wrapped_function)  # TODO: check whether caching messes us up here by giving the same AST for different mutations

        return wrapped_steps

    return _incrementally_improve_fuzzer
