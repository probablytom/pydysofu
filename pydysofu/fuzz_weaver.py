...
class FuzzingAspect:
    ...
    def reverse_fuzzer(self, attribute):
        attribute.func_code = attribute.__code_history__.pop()
...
def fuzzer(func):
    '''
    A decorator to indicate that a function can be applied as a fuzzing aspect.
    Turns it into an executable class instance.
    '''

    class AutoGeneratedFuzzingAspect(FuzzingAspect):
        def fuzzer(self, steps, context):
            return self.injected_fuzzer(steps, context)

        def __call__(self, steps, context):
            return self.fuzzer(steps, context)

    aspect = AutoGeneratedFuzzingAspect()
    aspect.injected_fuzzer = func
    return aspect
